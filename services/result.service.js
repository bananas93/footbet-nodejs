...
const findAll = async (tournament) => {
  ...
  const result = await db.sequelize.query(`SELECT \`result\`.\`id\`, \`result\`.\`user_id\` AS \`userId\`, COUNT(\`Result\`.\`id\`) AS \`matches\`, SUM(\`goals5\`) AS \`goals5\`, SUM(\`difference\`) AS \`difference\`, SUM(\`result\`) AS \`result\`, SUM(\`score\`) AS \`score\`, SUM(\`all\`) AS \`all\`, @curRank := @curRank + 1 AS \`rank\`, \`user\`.\`name\` AS \`user_name\` FROM \`results\` AS \`result\` LEFT OUTER JOIN \`users\` AS \`user\` ON \`result\`.\`user_id\` = \`user\`.\`id\` WHERE \`result\`.\`tournament_id\` = ${tournament} GROUP BY\`userId\`;`, { type: db.sequelize.QueryTypes.SELECT });
